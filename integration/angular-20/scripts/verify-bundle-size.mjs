...
const BUDGETS = {
  // Max size for any single JS chunk (main bundle + lib chunk combined)
  maxInitialBundleKb: 800,
  // Max total size of all JS in the dist
  maxTotalJsKb: 1500,
  // Max total size of all CSS in the dist
  maxTotalCssKb: 800,
};
...
if (jsFiles.length === 0) {
  // Fallback: try without /browser sub-folder (Angular <17 output structure)
...
if (largestJs.size / 1024 > BUDGETS.maxInitialBundleKb) {
  console.error(
    `✘ BUDGET EXCEEDED: Largest JS chunk ${kib(largestJs.size)} KiB > ${BUDGETS.maxInitialBundleKb} KiB`,
  );
  failed = true;
}
...
if (totalJs / 1024 > BUDGETS.maxTotalJsKb) {
  console.error(
    `✘ BUDGET EXCEEDED: Total JS ${kib(totalJs)} KiB > ${BUDGETS.maxTotalJsKb} KiB`,
  );
  failed = true;
}
...
if (totalCss / 1024 > BUDGETS.maxTotalCssKb) {
  console.error(
    `✘ BUDGET EXCEEDED: Total CSS ${kib(totalCss)} KiB > ${BUDGETS.maxTotalCssKb} KiB`,
  );
  failed = true;
}
